# pysbml SBML L3v2 Spec Compliance

**Spec:** SBML Level 3 Version 2 Core Release 2 (29 March 2019)
**Implementation:** `src/pysbml/` (parse layer: `parse/`, transform layer: `transform/`)
**Tests:** `tests/test_import.py`, test assets `tests/assets/` (cases 00001–01822)

**Scope:** L3v2 files only. L1/L2 files exist in the test suite and use the same parser; divergences
noted only where they affect L3v2 semantics.

---

## TODO

- [x] Read spec §4.1–4.12 (all core element types)
- [x] Read parse layer (`parse/l1.py`, `parse/data.py`)
- [x] Read transform layer (`transform/__init__.py`, `transform/data.py`)
- [x] Read test harness (`tests/test_import.py`)
- [x] Write decision trees + compliance entries for each element type
- [x] Write divergence index and summary compliance table
