...
| D4  | `SPEC_SILENT`    | Reaction        | `rxn_had_compartment` heuristic: spec says kinetic law unit is extent/time. pysbml auto-detects traditional concentration/time laws (those containing a compartment symbol) and auto-corrects by removing the compartment from the kinetic law and multiplying stoichiometry by it. Not described in spec. **Multi-compartment scope gap:** for reactions involving species from multiple compartments (S1 in V1, S2 in V2) where only V1 appears in the kinetic law, the flag fires and ALL species stoichiometries are multiplied by their respective compartments (S1 stoich × V1, S2 stoich × V2). S2's V2 factor is spurious and produces a wrong ODE for S2. The 833 D4 test cases do not include multi-compartment reactions, so this failure mode is untested.                                             |
...
- D4 + multi-compartment reactions: the heuristic's stoichiometry correction applies each species' own compartment even when only one compartment appeared in the kinetic law. Not in the 833 D4 test cases. See extended D4 note above.
